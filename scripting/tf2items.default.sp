...
{
	name = "[TF2Items] Natives for Default Weapons",
	author = "Thrawn, asherkin",
	description = "TF2Items interface, provides default unlockable weapons",
...
#endif
{
	RegPluginLibrary("tf2items.default");

	CreateNative("TF2Items_GiveWeapon", Native_GiveWeapon);
	CreateNative("TF2Items_GiveWeaponEx", Native_GiveWeaponEx);
...
public Native_GiveWeapon(Handle:hPlugin, iNumParams)
{
	new iClient = GetNativeCell(1);
	new iIIdx = GetNativeCell(2);
	LogMessage("Giving %i to %N", iIIdx, iClient);
	GiveWeapon(iClient, iIIdx);
}

//lm_SetClientLevel(iClient, iLevel);
public Native_GiveWeaponEx(Handle:hPlugin, iNumParams)
{
	new iClient = GetNativeCell(1);
	new iIIdx = GetNativeCell(2);

	GiveWeaponEx(iClient, iIIdx);
}
...
stock GiveWeaponEx(client, weaponLookupIndex) {
	new weaponSlot;
	new String:formatBuffer[32];
	...
	PrintToChat(client, "[SM] Respawn or touch a locker to recieve your new weapon.");
	g_hItems[client][weaponSlot] = weaponLookupIndex;
}

stock GiveWeapon(client, weaponLookupIndex) {
	new weaponSlot;
	new String:formatBuffer[32];
	Format(formatBuffer, 32, "%d_%s", weaponLookupIndex, "slot");
	new bool:isValidItem = GetTrieValue(g_hItemInfoTrie, formatBuffer, weaponSlot);
